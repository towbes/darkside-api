...
bool DarksideAPI::GetEntityList(LPVOID lpBuffer) {
    ...
    entityListAPI_t* tmpEntList = new entityListAPI_t();
    ...
    for (int i = 0; i < 2000; i++) {
        ...
        unsigned char* ptrEntNameShmBytePtr = reinterpret_cast<unsigned char*>(ptrEntName);
        //Offsets for the shared memory
        int entoffset = sizeof(entity_t) * i;
        int nameoffset = sizeof(entName_t) * i;
        ptrShmBytePtr += entoffset;
        ptrEntNameShmBytePtr += nameoffset;
        ...
        if (objId != 0) {
            ...
            memset(&tmpEntList->entList[i], 0, sizeof(entityInfoAPI_t));
        }

    }


    //Copy the entire EntityList to buffer
    memcpy(lpBuffer, tmpEntList, sizeof(entityListAPI_t));
